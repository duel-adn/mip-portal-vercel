...
export function useDateTime(initialTime) {
    ...
    const updateDate = (dateAsNumber) => {
        if (dateAsNumber) {
            const reqDate = new Date(dateAsNumber)
            const newDate = new Date(reqDate.getFullYear(), reqDate.getMonth(), reqDate.getDate(),
                date.getHours(), date.getMinutes())
            setDate(newDate)
        }
    }

    return [date, setDate, updateDate, updateTime]
}