...
export function translateUnixDate(locale, unixTime) {
...
export function translateLocalDateTime(locale, dateTime) {
    return dateTime ? new Intl.DateTimeFormat(locale ?? 'it-IT',
        {
            dateStyle: 'medium',
...
export function pad2(n) {
    return n >= 0 ? (n > 9 ? String(n) : '0' + n) : null
}

export function toISOLocalDate(date) {
    return date ? `${date.getFullYear()}-${pad2(date.getMonth() + 1)}-${pad2(date.getDate())}` : null
}

export function toISOLocalTime(date, addSeconds) {
    return date ? (addSeconds ? `${pad2(date.getHours())}:${pad2(date.getMinutes())}:${pad2(date.getSeconds())}`
        : `${pad2(date.getHours())}:${pad2(date.getMinutes())}`) : null
}

export function translateDuration(locale, duration_s) {
    const daysT = locale === 'en' ? ["day", "days"] : ["giorno", "giorni"]
    const hoursT = locale === 'en' ? ["hour", "hours"] : ["ora", "ore"]
    if (typeof duration_s === "number") {
        ...
        const daysDesc = days > 0 ?
            `${days} ${daysT[Number(days > 1)]}` : null
        const hoursDesc = hours > 0 ?
            `${hours} ${hoursT[Number(hours > 1)]}` : null
