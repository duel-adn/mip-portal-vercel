...
export function translateItineraryDescription(locale, itinerary) {
    ...
    const pictogram = itinerary.mode === MIPPlanMode.publicTransport ? createTransitPictogram(locale, itinerary) : null
    const walkTime = itinerary.mode === MIPPlanMode.publicTransport ? translateDuration(locale, itinerary.walkTime_s) : null
    const walkDistance = itinerary.mode === MIPPlanMode.publicTransport  ? translateDistance(locale, itinerary.walkDistance_m) : null
    return {
        ...modeTranslation,
        pictogram: pictogram,
