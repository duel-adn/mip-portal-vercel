...
import { MIPItineraryColors, MIPPlanMode } from "./MIPPlannerAPI"
...
function convertLeg(leg) {
    ...
    /*
    const routeColor = LegColor[mode] || "#222" // '#' + (leg.routeColor ?? (leg.transitLeg ? "0061B2" : "222")).toLowerCase()
    const routeTextColor = "white" // '#' + (leg.routeTextColor ?? "fff")
    const borderColor = routeColor // (routeColor === 'white' || routeColor === '#fff' || routeColor === '#ffffff') ? '#bbb' : routeColor
    */
...
// Colori usati dai diversi tipi di Leg
// L'identificare deve coincidere con un valore di LegMode
//
export const LegColor = {
    "Walk": "#222",
    "Vehicle": "#555",
    "Bycicle": "red",
    "Bus": "navy",
